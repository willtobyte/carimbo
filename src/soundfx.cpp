...
soundfx::soundfx(const std::string &filename) {
  ...
  if (ov_open_callbacks(fp.get(), vf.get(), nullptr, 0, PHYSFS_callbacks) < 0) [[unlikely]] {
    panic("[ov_open_callbacks] error while opening file");
  }
  ...
  if (!info) [[unlikely]] {
    panic("[ov_info] failed to retrieve OggVorbis info");
  }
  ...
  do {
    ...
    if (offset < 0) [[unlikely]] {
      panic("[ov_read] error while reading file: {}, error: {}", filename, ov_strerror(offset));
    }
