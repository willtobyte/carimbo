...
std::shared_ptr<particlebatch> particlefactory::create(const std::string& kind, float x, float y) const {
  ...
  c->yveld = std::uniform_real_distribution<float>(yvel.value("start", .0f), yvel.value("end", .0f));
  c->gxd = std::uniform_real_distribution<float>(gx.value("start", .0f), gx.value("end", .0f));
  c->gyd = std::uniform_real_distribution<float>(gy.value("start", .0f), gy.value("end", .0f));
  c->lifed = std::uniform_real_distribution<float>(life.value("start", 1.0f), life.value("end", 1.0f));
  c->alphad = std::uniform_int_distribution<unsigned int>(alpha.value("start", 255u), alpha.value("end", 255u));
  c->scaled = std::uniform_real_distribution<float>(scale.value("start", 1.0f), scale.value("end", 1.0f));
  c->rotforced = std::uniform_real_distribution<float>(rforce.value("start", .0f), rforce.value("end", .0f));
  c->rotveld = std::uniform_real_distribution<float>(rvel.value("start", .0f),   rvel.value("end", .0f));
