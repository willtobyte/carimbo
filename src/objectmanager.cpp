...
std::shared_ptr<object> objectmanager::create(const std::string& kind, std::optional<std::reference_wrapper<const std::string>> scope, bool manage) {
  ...
  for (const auto& item : j["animations"].items()) {
    ...
    const auto effect = a.contains("effect") ? _resourcemanager->soundmanager()->get(std::format("blobs/{}{}.ogg", scope ? std::format("{}/", scope->get()) : std::string(), a.at("effect").template get<std::string>())) : nullptr;
    const auto next = a.contains("next") ? std::make_optional(a.at("next").template get<std::string>()) : std::nullopt;
