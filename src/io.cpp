...
std::vector<std::uint8_t> io::read(std::string_view filename) {
  ...
  if (result != length) [[unlikely]] {
    throw std::runtime_error(std::format("[PHYSFS_readBytes] error reading file: {}, expected {} bytes but read {}, error: {}", filename, amount, result, PHYSFS_getErrorByCode(PHYSFS_getLastErrorCode())));
  }
...
std::vector<std::string> io::enumerate(std::string_view directory) {
  ...
  if (!ptr) [[unlikely]] {
    throw std::runtime_error(std::format("[PHYSFS_enumerateFiles] error while enumerating directory: {}, error: {}", directory, PHYSFS_getErrorByCode(PHYSFS_getLastErrorCode())));
  }
